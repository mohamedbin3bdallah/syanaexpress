	...
	*/
	function checkToken($refresh =0)
    {
        ...
        if (isset($api_token) && !empty($api_token)) {
          $user = $this->Api_model->getSingleRow('user', array('api_token' => $api_token));
		  if ($user->api_token_time > time() && $refresh==0){
              return 1;
          }
		  elseif($refresh==1 && $user){
              return 1;
          }
		  else{
              return 0;
	...
	*/
    ...
    public function checkUserKey($user_key, $user_id)
    {
        ...
        if (!$getUser) {
            ...
            $this->api->api_message(401, $this->translate('LOGIN', $lan));
        }
    }
...
//                      die;
    ...
    public function signIn()
    {
        ...
        $chkUser->company_name = null;
        $chkUser->attachments = null;
        ...
        if ($chkUser) {
            if ($role == 1) {
                ...
                if ($checkArtist) {
                    ...
                    $chkUser->company_name = $checkArtist->company;

                    $attachments = $this->Api_model->getArtistAttachs($checkArtist->id);

                    if($attachments){
                        $chkUser->attachments = $attachments;
                    }
            ...
            $where = array('user_id' => $chkUser->user_id);
			$datadevice['api_token'] = $this->createToken(60);
			$datadevice['api_token_time'] = strtotime('+1 year');
            $updateUser = $this->Api_model->updateSingleRow(USR_TBL, $where, $datadevice);

            $datatag = 'data';
			$chkUser->api_token = $datadevice['api_token'];
			$chkUser->api_token_time = $datadevice['api_token_time'];
    ...
    public function refreshToken(){
        ...
        if($this->checkToken(1)) {
            ...
            if(!$user){
                return $this->api->api_message(0, $this->translate('LOGINFAIL', $lan));
            }

            $where = array('api_token' => $api_token);
            $datadevice['api_token'] = $this->createToken(60);
            $datadevice['api_token_time'] = strtotime('+1 year');
            ...
            unset($user->password);
            $datatag = 'data';
            $user->api_token = $datadevice['api_token'];
            $user->api_token_time = $datadevice['api_token_time'];

            return $this->api->api_message_data(1, $this->translate('LOGINSUCCESSFULL', $lan), $datatag, $user);
        } else {
            $this->api->api_message(401, $this->translate('LOGIN', $lan));
        }
    }
...
//                }
    ...
    public function getCategories (){
        ...
        $gategories = $this->Api_model->getAllDataWhere($where, 'category');

        $this->api->api_message_data(1, $this->translate('ALL_CAT', $lan), 'data', $gategories);
...
//        }
    ...
    public function submitOrder()
    {
        ...
        $user_address = $this->Api_model->getSingleRow('user_addresses', array('user_id' => $data['user_id'], 'id' => $data['user_address_id']));

        if(!$user_address){
            return $this->api->api_message(0, $this->translate('USER_ADDRESS_NOT_VALID', $lan));
        }
	...
	*/
    public function getArtistWalletTransactions()
    {
        ...
        $select = 'artist_wallet_transactions.amount,artist_wallet_transaction_types.name_' . $lan . ' name, artist_wallet_transactions.created_at';
	...
	*/
    public function redeemArtistLoyaltyPoints()
    {
		...
		if($this->checkToken()) {
        $artist_id = $this->input->post('artist_id', TRUE);
        $point_reward_id = $this->input->post('redeemArtistLoyyaltyPoints', TRUE);
        ...
        if ($artist_points) {
            if ($point_reward->points_count <= $artist_points->points) {
                ...
                $this->api->api_message(0, $this->translgetArtistLoyaltyPointTransactionsate('REWARD_LARGER_THAN_ARTIST_POINTS', $lan));
            }
        } else {
    ...
    public function getArtistProfit()
    {
        ...
        $artist_wallet_transactions['profit'] = $this->Api_model->getArtistProfit($artist_id, $from, $to);
    ...
    public function assignArtistToBookingOrder()
    {
        ...
        $checkUser = $this->Api_model->getSingleRow('user', array('user_id' => $getBooking->user_id));
        ...
        if($this->checkToken()) {
            ...
            if($getBooking) {
                ...
                if ($updateBooking) {
                    ...
                    $getArtist= $this->Api_model->getSingleRow('artist', array('id'=>$getBooking->artist_id));

                    $logdata['booking_order_id'] = $booking_id;
                    $logdata['log_type_id'] = 2;
                    $logdata['status_id'] = $data['status_id'];
                    $logdata['details'] = "";
                    $logdata['created_by'] = $getArtist->user_id;
                    $bookingOrder = $this->Api_model->insertGetId("booking_orders_logs", $logdata);
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    # Committing data to the database.
                    $this->db->trans_commit();
                    return $this->api->api_message(1, $this->translate('ACTION_DONE_SUCCESSFULLY', $lan));
        ...
        if($this->checkToken()) {
            ...
            if($getBooking) {
                ...
                if ($updateBooking) {
                    ...
                    $getArtist= $this->Api_model->getSingleRow('artist', array('id'=>$artist_id));
    ...
    public function addServiceToOrder(){
        ...
        if ($booking && $category) {
            ...
            $data['cost_per_item']= $category->price;
...
//        die;
        ...
        if ($use_code) {
            ...
            if ($updateRecord) {
                ...
                $datatag = 'data';
                $get_user= $this->Api_model->getUserData(array('user_id' => $user_id));
                unset($get_user->password);

                $get_user->artist_id = null;
                $get_user->company_name = null;
                $get_user->attachments = null;
                ...
                if ($userRole == 1) {
                    ...
                    $get_user->artist_id=$get_artist->id;
                    $get_user->company_name = $get_artist->company;
...
//                      die;
                    ...
                    $attachments = $this->Api_model->getArtistAttachs($get_artist->id);

                    if($attachments){
                        $get_user->attachments = $attachments;
                    }
